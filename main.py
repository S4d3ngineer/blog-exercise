...
class BlogPost(db.Model):
    # Setting __tablename__ class attribute overrides table name
...
def admin_only(func):
    # wrapping via functools.wraps preserves information about the original function
...
def register():
    ...
    if form.validate_on_submit():
        # hashing and salting password before passing it to database
...
def login():
    ...
    if form.validate_on_submit():
        ...
        if not user:
            flash("This email address does not exist in our database")
...
def show_post(post_id):
    ...
    if form.validate_on_submit() and current_user.is_authenticated:
        ...
        flash("You have to be logged in in order to comment.")
